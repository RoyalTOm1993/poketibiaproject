...
function setupExtraHotkeys(combobox)
  ...
  addExtraHotkey("fullCombo", "Full Attack", function(repeated)
    if not repeated then
		for i = 1, 12 do
  ...
  local move = 1
  local minForFull = 0
  addExtraHotkey("sequencialCombo", "Sequencial Attack", function(repeated)
    if not repeated then
		modules.game_console.sendMessage("#s m"..move)
		move = move + 1
		if move == 12 then
			move = 1
		end
	else
		minForFull = minForFull + 1
		if minForFull >= 30 then
			for i = 1, 12 do
				modules.game_console.sendMessage("#s m"..i)
			end
			minForFull = 0
