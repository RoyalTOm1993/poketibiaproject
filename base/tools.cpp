...
const char* getReturnMessage(ReturnValue value)
{
	switch (value) {
		...
		case RETURNVALUE_PLAYERWITHTHISNAMEISNOTONLINE:
			return "This trainer is offline.";
...
CombatType_t raceToCombatType(RaceType_t race) {
    switch (race) {
        case RACE_NORMAL:    return COMBAT_NORMALDAMAGE;
        case RACE_FIRE:      return COMBAT_FIREDAMAGE;
        case RACE_WATER:     return COMBAT_WATERDAMAGE;
        case RACE_ELECTRIC:  return COMBAT_ELECTRICDAMAGE;
        case RACE_GRASS:     return COMBAT_GRASSDAMAGE;
        case RACE_ICE:       return COMBAT_ICEDAMAGE;
        case RACE_FIGHTING:  return COMBAT_FIGHTINGDAMAGE;
        case RACE_POISON:    return COMBAT_POISONDAMAGE;
        case RACE_GROUND:    return COMBAT_GROUNDDAMAGE;
        case RACE_FLYING:    return COMBAT_FLYINGDAMAGE;
        case RACE_PSYCHIC:   return COMBAT_PSYCHICDAMAGE;
        case RACE_BUG:       return COMBAT_BUGDAMAGE;
        case RACE_ROCK:      return COMBAT_ROCKDAMAGE;
        case RACE_GHOST:     return COMBAT_GHOSTDAMAGE;
        case RACE_DRAGON:    return COMBAT_DRAGONDAMAGE;
        case RACE_DARK:      return COMBAT_DARKDAMAGE;
        case RACE_STEEL:     return COMBAT_STEELDAMAGE;
        case RACE_FAIRY:     return COMBAT_FAIRYDAMAGE;
...
RaceType_t combatToRace(CombatType_t combatType) {
    switch (combatType) {
        case COMBAT_NORMALDAMAGE:   return RACE_NORMAL;
        case COMBAT_FIREDAMAGE:     return RACE_FIRE;
        case COMBAT_WATERDAMAGE:    return RACE_WATER;
        case COMBAT_ELECTRICDAMAGE: return RACE_ELECTRIC;
        case COMBAT_GRASSDAMAGE:    return RACE_GRASS;
        case COMBAT_ICEDAMAGE:      return RACE_ICE;
        case COMBAT_FIGHTINGDAMAGE: return RACE_FIGHTING;
        case COMBAT_POISONDAMAGE:   return RACE_POISON;
        case COMBAT_GROUNDDAMAGE:   return RACE_GROUND;
        case COMBAT_FLYINGDAMAGE:   return RACE_FLYING;
        case COMBAT_PSYCHICDAMAGE:  return RACE_PSYCHIC;
        case COMBAT_BUGDAMAGE:      return RACE_BUG;
        case COMBAT_ROCKDAMAGE:     return RACE_ROCK;
        case COMBAT_GHOSTDAMAGE:    return RACE_GHOST;
        case COMBAT_DRAGONDAMAGE:   return RACE_DRAGON;
        case COMBAT_DARKDAMAGE:     return RACE_DARK;
        case COMBAT_STEELDAMAGE:    return RACE_STEEL;
        case COMBAT_FAIRYDAMAGE:    return RACE_FAIRY;
...
double calculateDamageMultiplier(RaceType_t attackerRace, RaceType_t firstRaceTarget, RaceType_t secondRaceTarget, bool isNightmareWorld /*=false*/, bool isPvP /*=false*/) {
    const auto& effectiveness = CONST_EFFECTIVENESS[attackerRace];

    // Função para determinar o multiplicador com base na eficácia contra um único tipo
    auto getMultiplier = [&](RaceType_t race) {
        if (std::find(effectiveness.noEffect.begin(), effectiveness.noEffect.end(), race) != effectiveness.noEffect.end()) {
			if (isPvP) 
				return 0.4;
            return isNightmareWorld ? 0.5 : 0.0;
        }
        if (std::find(effectiveness.ineffective.begin(), effectiveness.ineffective.end(), race) != effectiveness.ineffective.end()) {
            return 0.5;
        }
        if (std::find(effectiveness.effective.begin(), effectiveness.effective.end(), race) != effectiveness.effective.end()) {
            return 2.0;
        }
        return 1.0;
    };
    ...
    if (secondRaceTarget == RACE_NONE) {
        return getMultiplier(firstRaceTarget);
    }

    // Caso de duplo elemento
    double multiplierFirst = getMultiplier(firstRaceTarget);
    double multiplierSecond = getMultiplier(secondRaceTarget);

    if (multiplierFirst == 2.0 && multiplierSecond == 2.0) {
        return 2.0;
    }
    if (multiplierFirst == 2.0 || multiplierSecond == 2.0) {
        return 1.75;
    }
    if (multiplierFirst == 1.0 && multiplierSecond == 1.0) {
        return 1.0;
    }
    if ((multiplierFirst == 0.5 && multiplierSecond == 1.0) || (multiplierFirst == 1.0 && multiplierSecond == 0.5)) {
        return 0.75;
    }
    if (multiplierFirst == 0.5 && multiplierSecond == 0.5) {
        return 0.5;
    }
    if (multiplierFirst == 0.0 || multiplierSecond == 0.0) { // Imune
		if (isPvP)
			return 0.4;
        return isNightmareWorld ? 0.5 : 0.0;
    }
