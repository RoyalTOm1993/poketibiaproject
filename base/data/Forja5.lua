function onUse(cid, item, fromPosition, itemEx, toPosition)
    local itemToRemove = 13215 -- ID do item a ser removido
    local itemToAdd = 13559 -- ID do item a ser adicionado

    local player = Player(cid)
    local playerItemCount = player:getItemCount(itemToRemove)
    local amountToAdd = math.floor(playerItemCount / 100) -- Quantidade de itemToAdd a ser adicionada (calculada a partir do itemToRemove)
    local amountToRemove = amountToAdd * 100 -- Quantidade de itemToRemove a ser removida
