...
registerMonsterType.wild = function(mtype, mask)
	if mask.wild then
		if mask.wild.health then
			mtype:wildHealth(mask.wild.health)
		end
		if mask.wild.maxHealth then
			mtype:wildMaxHealth(mask.wild.maxHealth)
		end
		if mask.wild.speed then
			mtype:wildBaseSpeed(mask.wild.speed)
		end
	end
end
...
registerMonsterType.race2 = function(mtype, mask)
	if mask.race2 then
		mtype:race2(mask.race2)
	end
end
...
registerMonsterType.evolutions = function(mtype, mask)
	if type(mask.evolutions) == "table" then
		for k, v in pairs(mask.evolutions) do
			-- monsterType:addEvolution(name, level, chance, {{stoneId, stoneCount}, {...}})
			mtype:addEvolution(v.pokeName, v.level, v.chance, v.stones ) 
		end
	end
end
		...
		for _, attack in pairs(mask.attacks) do
			...
			if attack.name then
				if attack.name == "melee" then
					...
					if attack.passive then
						spell:setPassive(attack.passive)
					end
					if attack.interval then
						spell:setInterval(attack.interval)
					end
...
registerMonsterType.moves = function(mtype, mask)
	if type(mask.moves) == "table" then
		for _, attack in pairs(mask.moves) do
			local spell = MonsterSpell()
			if attack.name then
				if attack.name == "melee" then
					spell:setType("melee")
