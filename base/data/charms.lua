...
return true
    ...
    "Você acaba de adicionar tempo a: %s, novo tempo: %s",
    charm.name, convertTime(remainingTime + charm.time)))
    item:remove(1)
    return true
    end
    ...
    player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, string.format(
    "Você acaba de ativar um %s, com durao de: %s",
    ...
    player:setStorageValue(charmStorage, charm.time + os.time())
    player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, string.format(
    "Você acaba de ativar um %s, com duração de: %s",
    charm.name, convertTime(charm.time)))
    item:remove(1)
    return true
end