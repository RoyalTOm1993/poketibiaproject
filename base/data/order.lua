...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if not summon then
        player:sendCancelMessage("Você não tem um Pokémon ativo!")
        return false
    end
    
    summon:setPokeStopped(true) -- Faz o Pokémon parar
    player:say("Fique parado " .. summon:getName() .. "!", TALKTYPE_SAY)
...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if target:isItem() then
        if target:isItem() then
			...
			local phrases = {
				"Vá até lá " .. summonName .. "!",
				summonName .. ", mova-se!",
				"Rápido " .. summonName .. ", ande!",
				"Em movimento, " .. summonName .. "!",
				summonName .. ", não fique parado!"
			}
    ...
    if target:isCreature() then
		if target == player then
            ...
            player:say(("Deixe-me subir %s!"):format(summon:getName()), TALKTYPE_ORANGE_1)
		end
	end
	return true
end
