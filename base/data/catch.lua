...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if math.random(0, 10000) <= chance or INFOS_CATCH[string.lower(name)] and (pontos+player:getCatchPoints(name)) >= INFOS_CATCH[string.lower(name)].pontos then
        ...
        -- Delay de 1 segundo para o efeito visual 179
        addEvent(doPlayerSendEffect, delayMessage + 1000, player:getId(), 179)
        ...
        -- Delay de 1 segundo para o efeito visual 167
        addEvent(doPlayerSendEffect, delayMessage + 1000, player:getId(), 167)
        addEvent(function() 
