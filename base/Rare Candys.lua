function onUse(player, item, fromPosition, target, toPosition)
    ...
    local cfg = {
        ...
        max = 200,      --Boost máximo do seu servidor.
        chance = 40,   --Chance de acertar, em %
        boost_fail = 1,  --A partir de quantos boosts poderá falhar.
    }
...
local boost = target:getSpecialAttribute("pokeLevel") or 0
	...
	local chanceFail = math.random(1, 100)
        if chanceFail <= cfg.chance then
           ...
           player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, "Sua rare candy falhou!")
           item:remove(1)
