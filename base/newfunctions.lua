...
balls = {
	...
	premier = {emptyId = 26683, usedOn = 26666, usedOff = 26678, effectFail = 1569, effectSucceed = 1570, missile = 74, effectRelease = 1568, chanceMultiplier = 4.0},
...
-- =====================================================================
-- NOME DE EXIBIÇÃO (nickname -> fallback com remoção de prefixos) + TRADUÇÃO
-- =====================================================================
local NAME_PREFIXES = {
...
local function getDisplayNameFromBall(ball)
  if not ball then return "" end
  local nick = ball.getSpecialAttribute and (
      ball:getSpecialAttribute("nickname") or
      ball:getSpecialAttribute("nick") or
      ball:getSpecialAttribute("customName") or
      ball:getSpecialAttribute("givenName")
    ) or nil
  if nick and nick ~= "" then return nick end
...
-- Tradução mínima PT/EN (storage 9000)
...
local translations = {
  ["pt-BR"] = {
    ["%s, I choose you!"] = "%s, eu escolho você!",
    ["%s, return!"] = "%s, volte!",
    ["Sorry, not possible. Your summon is dead."] = "Desculpe, não é possível. Seu Pokémon está desmaiado.",
    ["Sorry, not possible while on ride."] = "Desculpe, não é possível enquanto estiver montado.",
    ["Sorry, not possible while on fly."]  = "Desculpe, não é possível enquanto estiver voando.",
    ["Sorry, not possible while on surf."] = "Desculpe, não é possível enquanto estiver surfando.",
    ["Sorry, not possible while on event."] = "Desculpe, não é possível durante o evento.",
    ["Sorry, not possible while evolving."] = "Desculpe, não é possível enquanto estiver evoluindo.",
    ["You are not flying."] = "Você não está voando."
  },
  ["en-US"] = {
    ["%s, I choose you!"] = "%s, I choose you!",
    ["%s, return!"] = "%s, return!",
    ["Sorry, not possible. Your summon is dead."] = "Sorry, not possible. Your summon is dead.",
    ["Sorry, not possible while on ride."] = "Sorry, not possible while on ride.",
    ["Sorry, not possible while on fly."]  = "Sorry, not possible while on fly.",
    ["Sorry, not possible while on surf."] = "Sorry, not possible while on surf.",
    ["Sorry, not possible while on event."] = "Sorry, not possible while on event.",
    ["Sorry, not possible while evolving."] = "Sorry, not possible while evolving.",
    ["You are not flying."] = "You are not flying."
  }
}
...
function doReleaseSummon(cid, pos, effect, message, missile)
	...
	if health <= 0 then
		player:sendCancelMessage(trServer(player, "Sorry, not possible. Your summon is dead."))
	...
	if monster ~= nil then
		...
		-- >>> Fala com nome “exibido” (nickname/fallback) e tradução
		if message then
			local displayName = getDisplayNameFromBall(ball)
			player:say(string.format(trServer(player, "%s, I choose you!"), displayName), TALKTYPE_MONSTER_SAY)
		end
		...
		-- nickname visual do monstro (mantém sua lógica de +boost)
...
function doRemoveSummon(cid, effect, uid, message, missile)
	...
	if message then
		local dispName = getDisplayNameFromSummon(summon)
		player:say(string.format(trServer(player, "%s, return!"), dispName), TALKTYPE_MONSTER_SAY)
	end
...
function Player:flyUp()
	if self:isOnFly() then
		...
		self:sendCancelMessage(trServer(self, "You are not flying."))
		return false
	end
end
...
function Player:addTokens(number)
	...
	-- (mantido) soma controlada em outro ponto
	return false
end
...
function Player:isSummonBlocked()
	...
	if self:getStorageValue(storageRide) == 1 then
		self:sendCancelMessage(trServer(self, "Sorry, not possible while on ride."))
		return true
	end

	if self:getStorageValue(storageFly) == 1 then
		self:sendCancelMessage(trServer(self, "Sorry, not possible while on fly."))
		return true
	end

	if self:getStorageValue(storageSurf) > 0 then
		self:sendCancelMessage(trServer(self, "Sorry, not possible while on surf."))
		return true
	end

	if self:getStorageValue(storageEvent) > 0 then
		self:sendCancelMessage(trServer(self, "Sorry, not possible while on event."))
		return true
	end

	if self:getStorageValue(storageEvolving) == 1 then
		self:sendCancelMessage(trServer(self, "Sorry, not possible while evolving."))
		return true
	end
