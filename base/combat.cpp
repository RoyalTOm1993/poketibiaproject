...
ReturnValue Combat::canDoCombat(Creature* attacker, Creature* target)
{
	...
	if (target->getSummonCount() > 0 || target->isMonsterGuardian()) {
		return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
	}

	if (attacker) {
	    Player* playerAttacker = nullptr;

	    if (attacker->getPlayer()) {
	        playerAttacker = attacker->getPlayer();
	    } else if (attacker->getMaster() && attacker->getMaster()->getPlayer()) {
	        playerAttacker = attacker->getMaster()->getPlayer();
	    }

	    if (playerAttacker) {
	        Creature* master = target->getMaster();
	        if (target->getDuelist() || (master && master->getDuelist())) {
	            Duelist* targetDuelist = target->getDuelist();
	            Duelist* masterDuelist = master ? master->getDuelist() : nullptr;

	            if ((targetDuelist && targetDuelist->getContractPlayer() > 0 && targetDuelist->getContractPlayer() != playerAttacker->getGUID()) || 
	                (masterDuelist && masterDuelist->getContractPlayer() > 0 && masterDuelist->getContractPlayer() != playerAttacker->getGUID())) {
	                return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
	            }
	        }
	    }
...
void Combat::setPlayerCombatValues(formulaType_t formulaType, double mina, [[maybe_unused]] double minb, double maxa, [[maybe_unused]] double maxb)
{
...
bool Combat::setStringParam(CombatParam_t param, std::string value)
{
	switch (param) {
		case COMBAT_PARAM_STRING_SPELLNAME: {
			params.spellName = value;
			return true;
		}
		default:
			return false;
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	if (casterPlayer) {
		Combat::checkLeech(casterPlayer, damage);
		if (!damage.critical && damage.origin != ORIGIN_CONDITION && (damage.primary.value < 0)) {
		...
		if (CreatureVector* creatures = tile->getCreatures()) {
			...
			for (Creature* creature : *creatures) {
				...
				if (!params.aggressive || (caster != creature && Combat::canDoCombat(caster, creature) == RETURNVALUE_NOERROR)) {
					...
					if ((damageCopy.primary.value < 0) && caster) {
					...
					if (damageCopy.primary.type != COMBAT_MANADRAIN) {
						...
						damageCopy.spellName = params.spellName;
					...
					if (success) {
						if (params.origin != ORIGIN_MELEE && damageCopy.primary.value != 0) {
							for (const auto& condition : params.conditionList) {
								if (caster == creature || !creature->isImmune(condition->getType())) {
									Condition* conditionCopy = condition->clone();
									conditionCopy->setParam(CONDITION_PARAM_OWNER, caster->getID());
