...
defaultComboKeysTable = {
  ...
  [13] = {key = 'PageUp', autoSend = true, itemId = nil, subType = nil, useType = nil, value = '#s h1'},
  [14] = {key = 'PageDown', autoSend = true, itemId = nil, subType = nil, useType = nil, value = '#s h2'},
}
...
function init()
  if not g_app.isMobile() then
    hotkeysButton = modules.client_topmenu.addLeftGameButton('hotkeysButton', tr('Hotkeys') .. ' (Ctrl+K)', '/images/topbuttons/hotkeys', toggle, false, 3)
  end
...
function unload()
	...
	for keyCombo,callback in pairs(boundCombosCallback) do
		if not mouseActionsbar[keyCombo] then
...
function hotkeyCaptureMouse(assignWindow, mousePosition, mouseButton)
	for i, mouseBtn in pairs(mouseActionsbar) do
		if mouseButton == mouseBtn then
			local comboPreview = assignWindow:getChildById('comboPreview')
			comboPreview:setText(tr('Current hotkey to add: %s', i))
			comboPreview.keyCombo = i
...
function addKeyCombo(keyCombo, keySettings, focus)
  ...
  if not hotkeyLabel then
	...
	if mouseActionsbar[keyCombo] then
		boundCombosCallback[keyCombo] = function() doKeyCombo(keyCombo) end
		g_mouse.bindPress(rootWidget, boundCombosCallback[keyCombo], mouseActionsbar[keyCombo])
	else
...
function removeHotkey()
	...
	local gameRootPanel = modules.game_interface.getRootPanel()
	configValueChanged = true
	if not mouseActionsbar[currentHotkeyLabel.keyCombo] then
