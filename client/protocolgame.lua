...
function ProtocolGame:onCallback(msg)
  local opcode = msg:getU8()
  for i, callback in pairs(monsterCallbacks) do
    if i == opcode then
      callback(self, msg)
      return true
    end
...
function ProtocolGame.registerCallback(opcode, callback)
  if monsterCallbacks[opcode] then
    error('opcode ' .. opcode .. ' already registered will be overriden')
  end

  monsterCallbacks[opcode] = callback
end

function ProtocolGame.unregisterOpcode(opcode)
  opcodeCallbacks[opcode] = nil
end
function ProtocolGame.unregisterCallback(opcode)
  monsterCallbacks[opcode] = nil
end
