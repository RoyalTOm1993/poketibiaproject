...
function UITable:onStyleApply(styleName, styleNode)
  for name, value in pairs(styleNode) do
    if (value == "" or value == nil)then
      print(string.format("UITable:onStyleApply - invalid value for style '%s' in '%s', id: %s, %s", name or "empty", styleName, self:getId(), self:getParent():getId() or "no parent"))
    end
    if value ~= false then
...
function UITable:addRow(data, height, custom)
  if not self.dataSpace then
  ...
  for colId, column in pairs(data) do
    ...
    if column.imageOffset then
      col:setImageOffset(column.imageOffset)
    end
    if column.checked ~= nil then
      col:setChecked(column.checked)
    end
    if column.permission ~= nil then
      col.permission = column.permission
    end
    table.insert(self.columns[rowId], col)
  end

  row.onDoubleClick = function(mousePos)
    return self:onDoubleClickRow(mousePos)
  end
...
function UITable:onDoubleClickRow(mousePos)
  return signalcall(self.onRowDoubleClick, self, self.selectedRow)
end
...
function UITable:setTableData(tableData)
  ...
  self.dataSpace:applyStyle({height = self:getHeight() - headerHeight})
end
