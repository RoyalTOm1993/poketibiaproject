...
function create(currentOutfit, outfitList, mountList, wingList, auraList, shaderList, healthBarList, manaBarList)
  ...
  window.preview.options.showMount:setVisible(not g_game.getFeature(GamePlayerMounts))
  ...
  window.appearance.settings.mount:setVisible(not g_game.getFeature(GamePlayerMounts))
  ...
  window.configure.mount:setVisible(not g_game.getFeature(GamePlayerMounts))
...
function updatePreview()
  ...
  if not settings.showBars then
    ...
    if not g_game.getFeature(GamePlayerMounts) and settings.showMount and previewOutfit.mount > 0 then
