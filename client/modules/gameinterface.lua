...
function tryExit()
  ...
  exitWindow = displayGeneralBox(tr('Exit'),
    ...
    {
      { text = tr('For�ar Sa�da'), callback = exitFunc },
...
function onMouseGrabberRelease(self, mousePosition, mouseButton)
  ...
  if mouseButton == MouseLeftButton or mouseButton == MouseMidButton or mouseButton == MouseButton4 or mouseButton == MouseButton5 then
...
function onUseWith(clickedWidget, mousePosition)
  ...
  if isRevive(selectThingID) then
    local clickedWidget = modules.game_poketeam.getPokebar()
    local pokemon = clickedWidget:getChildByPos(mousePosition)
    if pokemon then
      print("Enviando interface")
      modules.game_poketeam.sendPokebarRevive(pokemon:getId(), selectedThing)
    end
  end
    ...
    if tile then
      if selectedThing:isFluidContainer() or selectedThing:isMultiUse() then
        if selectedThing:getId() == 3180 or selectedThing:getId() == 3156 then
          ...
          g_game.useWith(selectedThing, tile:getTopMultiUseThingEx(clickedWidget:getPositionOffset(mousePosition)),
            selectedSubtype)
        end
      else
        g_game.useWith(selectedThing, tile:getTopUseThing(), selectedSubtype)
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if lookThing then
    ...
    local copyPos = lookThing:getPosition()
    local text = string.format("Position(%d, %d, %d)", copyPos.x, copyPos.y, copyPos.z)

    menu:addOption("Copy Position", function() g_window.setClipboardText(text) end)
  end
  ...
  if creatureThing then
    ...
    if creatureThing:isLocalPlayer() then
      ...
      if creatureThing:isLocalSummon() then
        menu:addOption(tr('Particle Aura'), function() modules.pokemon_shaders.showModule() end)
      end
      ...
      if creatureThing:isNpc() then
        menu:addOption(tr('Talk'), function()
          ...
          if ((destPos.x >= myPos.x - 3) and (destPos.x <= myPos.x + 3) and (destPos.y >= myPos.y - 3) and (destPos.y <= myPos.y + 3)) then
            ...
            "Voc� n�o pode conversar com o npc, voc� est� muito longe")
          end
        end, shortcut)
      end
      ...
      if creatureThing:isPlayer() then
        ...
        if modules.game_console.isIgnored(creatureName) then
          menu:addOption(tr('Unignore') .. ' ' .. creatureName,
            function() modules.game_console.removeIgnoredPlayer(creatureName) end)
        else
          menu:addOption(tr('Ignore') .. ' ' .. creatureName,
            function() modules.game_console.addIgnoredPlayer(creatureName) end)
        end
        ...
        if localPlayerShield == ShieldNone or localPlayerShield == ShieldWhiteBlue then
          if creatureShield == ShieldWhiteYellow then
            menu:addOption(tr('Join %s\'s Party', creatureThing:getName()),
              function() g_game.partyJoin(creatureThing:getId()) end)
          else
            menu:addOption(tr('Invite to Party'), function() g_game.partyInvite(creatureThing:getId()) end)
          end
        elseif localPlayerShield == ShieldWhiteYellow then
          if creatureShield == ShieldWhiteBlue then
            menu:addOption(tr('Revoke %s\'s Invitation', creatureThing:getName()),
              function() g_game.partyRevokeInvitation(creatureThing:getId()) end)
          end
        elseif localPlayerShield == ShieldYellow or localPlayerShield == ShieldYellowSharedExp or localPlayerShield == ShieldYellowNoSharedExpBlink or localPlayerShield == ShieldYellowNoSharedExp then
          ...
          elseif creatureShield == ShieldBlue or creatureShield == ShieldBlueSharedExp or creatureShield == ShieldBlueNoSharedExpBlink or creatureShield == ShieldBlueNoSharedExp then
            menu:addOption(tr('Pass Leadership to %s', creatureThing:getName()),
              function() g_game.partyPassLeadership(creatureThing:getId()) end)
          else
            menu:addOption(tr('Invite to Party'), function() g_game.partyInvite(creatureThing:getId()) end)
          end
        end
      ...
      for name, opt in pairs(category) do
        if opt and opt.condition(menuPosition, lookThing, useThing, creatureThing) then
          menu:addOption(name, function()
            opt.callback(menuPosition,
              lookThing, useThing, creatureThing)
          end, opt.shortcut)
        end
      end
    end
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature,
  ...
  if useThing then
    local itemInfo = useThing:getItemInfo()
    -- print(itemInfo.ItemID, itemInfo.name, itemInfo.desc, itemInfo.pokeballInfo)
  end
...
function refreshViewMode()
  ...
  if classic then
    ...
    if g_app.isMobile() then
      ...
      gameMapPanel:setZoom(12)
      -- g_game.changeMapAwareRange(101, 101)
    end
...
function setupLeftActions()
  ...
  if gameLeftActions.fullhit then
    gameLeftActions.fullhit.onClick = function()
      for i = 1, 12 do
        modules.game_console.sendMessage("#s m" .. i)
      end
    end
  end
...
function getChatButton()
  return gameRootPanel.chatButton
end
