...
-- Foca o campo do chat com redundancia (evita que o mapa/WSAD roube o foco)
local function forceTextEditFocus()
  if not consolePanel or not consolePanel:isVisible() or not consoleTextEdit then return end
  consolePanel:raise()
  consoleTextEdit:setVisible(true)
  consoleTextEdit:setCursorVisible(true)
  consoleTextEdit:focus()
  addEvent(function()
    if consolePanel:isVisible() and consoleTextEdit then
      consoleTextEdit:focus()
    end
  end)
...
-- Handler do ENTER
local function onConsoleEnter()
  if not g_game.isOnline() then return end
  if not consolePanel or not consoleTextEdit then return end
...
-- Trava/Destrava o Console sem ocultar o chat / Enter Global no funciona para ocultar o Console se estiver chatLocked ativo
...
function init()
  ...
  g_keyboard.bindKeyDown('Enter', onConsoleEnter, gameRootPanel)
...
-- Alterna o modo de chat e ajusta a visibilidade do cursor.
function toggleChat()
...
-- Movimentação do Console com mouse
