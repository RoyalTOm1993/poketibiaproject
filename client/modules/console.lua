...
-- Reinicia o blink do cursor do TextEdit
local function restartCursorBlink()
  if not consoleTextEdit then return end
  if consoleTextEdit.resetCursorBlink then
    consoleTextEdit:resetCursorBlink()
  else
...
-- Trava/Destrava o Console sem ocultar o chat / Enter Global não funciona para ocultar o Console se estiver chatLocked ativo
  ...
  connect(consoleTextEdit, {
    onFocusChange = function(_, focused)
      if not focused and consolePanel:isVisible() then
        scheduleEvent(forceTextEditFocus, 50)
      end
    end
  })
