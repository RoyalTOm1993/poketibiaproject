...
local PERSIST_KEY_CONSOLE_HIDDEN = 'game_console/hidden' -- boolean
consolePanel = nil
...
local function handleGlobalEnter()
  ...
  if chatHidden then
    ...
    scheduleEvent(function()
      ...
      if consoleTextEdit.setFocusable then consoleTextEdit:setFocusable(true) end
      consoleTextEdit:setEnabled(true)
      consoleTextEdit:setVisible(true)
      if consoleTextEdit.raise then consoleTextEdit:raise() end
      if consoleTextEdit.grabKeyboard then consoleTextEdit:grabKeyboard() end
      consoleTextEdit:focus()
      if consoleTextEdit.setCursorPos then consoleTextEdit:setCursorPos(-1) end
    end, 80)
    return
  end
...
local function sendCurrentMessage()
  ...
  consolePanel:setVisible(not visible)
    -- PERSISTENCIA: salvar "hidden"
    if g_settings and g_settings.setBoolean then
      g_settings.setBoolean(PERSIST_KEY_CONSOLE_HIDDEN, visible == true)
    end
...
function init()
  ...
  if g_settings and g_settings.getBoolean then
    wasHidden = g_settings.getBoolean(PERSIST_KEY_CONSOLE_HIDDEN) or false
  end

  addEvent(function()
    hideAndShowChat(wasHidden)
  end)
...
function disableChat(temporarily)
  ...
  if consoleTextEdit.ungrabKeyboard then consoleTextEdit:ungrabKeyboard() end
  consoleTextEdit:clearText()
