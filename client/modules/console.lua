...
local function focusConsoleTextEdit()
...
function init()
  ...
  consoleTextEdit.onFocusChange = function(_, focused)
    if focused and consolePanel:isVisible() and consoleTextEdit.setCursorPos then
  ...
  local wasHidden = g_settings.getBoolean and (g_settings.getBoolean(PERSIST_KEY_CONSOLE_HIDDEN) or false) or false
  addEvent(function() hideAndShowChat(wasHidden) end)
...
function enableChat(temporarily)
  ...
  consoleTextEdit:clearText()
  scheduleEvent(focusConsoleTextEdit, 50)
