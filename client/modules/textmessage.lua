...
MessageTypes = {
  [MessageModes.Spell] = MessageSettings.consoleMove,
  [MessageModes.MonsterSay] = MessageSettings.consoleOrange,
  [MessageModes.MonsterYell] = MessageSettings.consoleOrange,
  [MessageModes.BarkLow] = MessageSettings.consoleMove,
  [MessageModes.BarkLoud] = MessageSettings.consoleMove,
...
local screenTargetsToHide = {
  highCenterLabel = true,
  middleCenterLabel = true,
  lowCenterLabel = true,
  privateLabel = true,
  statusLabel = true
}
  ...
  messagesPanel = g_ui.loadUI('textmessage', modules.game_interface.getRootPanel())
   ...
   for _, child in pairs(messagesPanel:recursiveGetChildren()) do
    if child:getId():match('Label') then
      child:setFont(screenMessageFont)
    end
  end
end
...
function displayMessage(mode, text)
  ...
  if msgtype.screenTarget then
    local chatHidden = modules.game_console and modules.game_console.isChatHidden and modules.game_console.isChatHidden()
    if not (screenTargetsToHide[msgtype.screenTarget] and chatHidden) then
