...
local questRefreshError = false -- Variável de controle para evitar spam de prints

function refreshQuests()
  if not g_game.isOnline() then 
    if not questRefreshError then
      print("Cliente não está online. refreshQuests não será executado.")
      questRefreshError = true
    end
    return 
  end
      ...
      if dispatcher[questData] and g_clock.millis() > dispatcher[questData] + callDelay + math.random(callDelay / 2) then
        ...
        scheduleEvent(function()
          if id and id ~= "" then
            ...
            if not questRefreshError then
              print("Falha ao atualizar a linha de quest: ID inválido ou ausente.")
              questRefreshError = true
            end
          end
